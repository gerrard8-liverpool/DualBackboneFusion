...
DATASETS=${DATASETS:-"caltech101 dtd eurosat fgvc_aircraft food101 imagenet oxford_flowers oxford_pets stanford_cars sun397 ucf101"}
...
for TARGET in $DATASETS; do
  ...
  echo "============================================================"
  echo "[Meta LODO B2N CLEAN] target=$TARGET"
  ...
  python scripts/dualenc_rpc/build_reliability_prior_cache.py \
    ...
    --out "$TARGET_DIR/meta_cache_exclude_${TARGET}.json" \
  ...
  python scripts/dualenc_rpc/eval_reliability_prior_cache.py \
    --manifest "$MANIFEST" \
    --project-root "$PROJECT_ROOT" \
    --cache-json "$TARGET_DIR/meta_cache_exclude_${TARGET}.json" \
  ...
  echo "[DONE] $TARGET_DIR/eval/rpc_eval_summary.md"
done

echo "[DONE] clean meta LODO B2N: $RUN_ROOT"
